...
# NUEVA CONFIGURACIÓN DE SEGURIDAD Y SESIÓN
# Lee la clave secreta de tu archivo .env
app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'SUPER_SECRETO_COCODI_2025_CAMBIAR') 
...
# --- LÍNEA DE DEBUG CRÍTICA (INSERTAR AQUÍ) ---
print(f"DEBUG: DATABASE_URL leída por Flask: {os.environ.get('DATABASE_URL')}")
# -----------------------------------------------
...
# --- FUNCIONES DE AUTENTICACIÓN Y DECORADORES ---

def get_user_by_id_login(user_id):
    """Obtiene un usuario de la BD por su ID de login (usuario)."""
    conn = get_db_connection()
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT id_usuario, nombre_completo, usuario, password_hash, rol FROM usuarios WHERE usuario = %s AND activo = TRUE;", (user_id,))
            user = cur.fetchone()
            if user:
                # El campo devuelto también debe ser 'usuario', no 'email'
                return dict(user) 
            return None
    finally:
        conn.close()
...
def get_user_by_id(user_id):
    """Obtiene un usuario de la BD por ID (PK: id_usuario)."""
    conn = get_db_connection()
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT id_usuario, nombre_completo, usuario, rol FROM usuarios WHERE id_usuario = %s AND activo = TRUE;", (user_id,))
            user = cur.fetchone()
...
def login_required(f):
    """Decorador que fuerza la autenticación para acceder a la ruta."""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:
            # Si la ruta es una API, devolvemos 401. 
...
def role_required(required_role):
    ...
    def wrapper(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            if 'user_role' not in session or session.get('user_role') not in required_role:
                print(f"!!! Intento de acceso a ruta restringida por rol: {session.get('user_role')}. Rol requerido: {required_role}")
                return jsonify({'error': f'Permiso denegado. Rol actual: {session.get("user_role")}.'}), 403
            return f(*args, **kwargs)
        return decorated_function
    return wrapper
...
# --- ENDPOINTS DE AUTENTICACIÓN (MODIFICADOS) ---

@app.route('/api/login', methods=['POST'])
def login():
    # CRÍTICO: Usamos request.form para obtener data, ya que JS enviará FormData.
    data = request.form
    ...
    # CAMBIO: Llamar a la nueva función de búsqueda por ID de usuario
    user = get_user_by_id_login(usuario) 

    if user and check_password_hash(user['password_hash'], password):
        session['user_id'] = user['id_usuario']
        session['user_role'] = user['rol']
        
        # Opcional: Actualizar el último acceso
        conn = get_db_connection()
        try:
            with conn.cursor() as cur:
                cur.execute("UPDATE usuarios SET ultimo_acceso = NOW() WHERE id_usuario = %s;", (user['id_usuario'],))
                conn.commit()
        except Exception:
            conn.rollback()
        finally:
...
def get_current_user():
    ...
    return jsonify({'user_id': None, 'rol': None})
...
@login_required
@role_required(['Admin', 'Responsable', 'Analista'])
def upload_file():
...
@login_required
@role_required(['Admin', 'Responsable', 'Analista']) # Aplica el rol más restrictivo a toda la función bimodal
...
@login_required
@role_required(['Admin', 'Analista']) # Aplica el rol más restrictivo a toda la función
    ...
    try:
        ...
        if request.method == 'DELETE':
            if session.get('user_role') not in ['Admin', 'Responsable']:
                return jsonify({'error': 'Permiso denegado. Solo Administrador y Aprobador pueden eliminar.'}), 403
...
@app.route('/api/recomendaciones', methods=['GET', 'POST']) # El GET es de lectura, pero POST es de escritura. Dejamos el decorador de escritura para proteger toda la función CRUD.
@login_required
@role_required(['Admin', 'Responsable', 'Analista'])
